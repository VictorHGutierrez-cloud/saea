...
import { PHASES, SOLUTION_BENEFITS, PROPOSAL, INVESTMENT, fmt } from '../data'
  ...
  return (
    <section id="solucao" ref={ref}>
      <div className="section-wrapper">
        ...
        {/* Included in Base Module - Highlight */}
        <motion.div
          initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.4, duration: 0.5 }}
          className="mb-12"
        >
          <div className="card p-8 bg-gradient-to-br from-stone-50 to-stone-100 border-2 border-stone-300">
            <div className="text-center mb-6">
              <h3 className="text-2xl font-bold text-stone-900 mb-2">Incluído no Módulo Base</h3>
              <p className="text-stone-600">
                Para as <strong className="text-stone-900">{INVESTMENT.employeesCovered.toLocaleString('pt-BR')} vidas contratadas</strong>, o módulo Core Enterprise inclui:
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {PROPOSAL.plans[0].included?.map((item, i) => (
                <div key={i} className="flex items-start p-4 bg-white rounded-lg border border-stone-200 shadow-sm">
                  <svg className="w-5 h-5 text-brand-primary mr-3 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
